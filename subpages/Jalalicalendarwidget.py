...
import subpages.Jalalicalendar as Jalalicalendar
...
def get_calendar(locale, fwday):
    ...
    if locale is None:
        return Jalalicalendar.TextCalendar(fwday)
    else:
        return Jalalicalendar.LocaleTextCalendar(fwday, locale)


class JCalendar(ttk.Frame):
    """
    ...
    jdatetime = Jalalicalendar.jdatetime.datetime
    timedelta = Jalalicalendar.jdatetime.timedelta
    ...
    def __init__(self, master=None, **kw):
        ...
        fwday = kw.pop('firstweekday', Jalalicalendar.SATURDAY)
        year = kw.pop('year', self.jdatetime.now().year)
        month = kw.pop('month', self.jdatetime.now().month)
        locale = kw.pop('locale', None)
        sel_bg = kw.pop('selectbackground', '#ecffc4')
        sel_fg = kw.pop('selectforeground', '#05640e')

        self.master = master
        self._date = self.jdatetime(year, month, 1)
    ...
    def __config_calendar(self):
        cols = self._cal.formatweekheader(7).split()
        self._calendar['columns'] = cols
        self._calendar.tag_configure('header', background='grey90')
        self._calendar.insert('', 'end', values=[ 'شنبه', 'یکشنبه', 'دوشنبه', 'سه‌شنبه', 'چارشنبه','پنجشنبه', 'جمعه'], tag='header')
    ...
    def _prev_month(self):
        ...
        self._date = self.jdatetime(self._date.year, self._date.month, 1)
    ...
    def _next_month(self):
        ...
        self._date = self._date + self.timedelta(
            days=Jalalicalendar.monthrange(year, month)[1] + 1)
        self._date = self.jdatetime(self._date.year, self._date.month, 1)
